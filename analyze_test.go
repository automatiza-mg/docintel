...
func TestAnalyzeDocument_Accepted(t *testing.T) {
	...
	location, err := client.AnalyzeDocument(t.Context(), AnalyzeDocumentParams{
	...
	// Sem Model/OutputFormat/Locale, aplica os padrões.
	if want := "/documentintelligence/documentModels/prebuilt-layout:analyze"; gotPath != want {
		t.Fatalf("path = %q, want %q", gotPath, want)
	}
	if got := gotQuery.Get("outputContentFormat"); got != "markdown" {
		t.Fatalf("outputContentFormat = %q, want markdown", got)
	}
	if got := gotQuery.Get("api-version"); got != "2024-11-30" {
		t.Fatalf("api-version = %q, want 2024-11-30", got)
	}
	if gotQuery.Has("locale") {
		t.Fatalf("locale should be omitted when empty, got %q", gotQuery.Get("locale"))
	}
}

func TestAnalyzeDocument_Params(t *testing.T) {
	t.Parallel()
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		gotQuery = r.URL.Query()
		w.Header().Set("Operation-Location", "https://example.com/op")
		w.WriteHeader(http.StatusAccepted)
	}))
	defer srv.Close()
	...
	if want := "/documentintelligence/documentModels/prebuilt-read:analyze"; gotPath != want {
		t.Fatalf("path = %q, want %q", gotPath, want)
	}
...
func TestAnalyzeDocument_Validation(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			params: AnalyzeDocumentParams{ContentType: "application/pdf"},
		},
		{
			name:   "missing content type",
			params: AnalyzeDocumentParams{Document: strings.NewReader("x")},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := client.AnalyzeDocument(t.Context(), tt.params)
			if !errors.Is(err, ErrInvalidAnalyzeRequest) {
				t.Fatalf("error = %v, want ErrInvalidAnalyzeRequest", err)
			}
		})
	}
}
